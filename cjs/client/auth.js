...
const auth$1 = require("../shared/auth.js");
...
async function parseErrorResponse(input) {
  ...
  try {
    const result = auth$1.OAuthErrorResponseSchema.parse(JSON.parse(body));
    const { error, error_description, error_uri } = result;
    const errorClass = errors.OAUTH_ERRORS[error] || errors.ServerError;
    return new errorClass(error_description || "", error_uri);
  } catch (error) {
    const errorMessage = `${statusCode ? `HTTP ${statusCode}: ` : ""}Invalid OAuth error response: ${error}. Raw body: ${body}`;
    return new errors.ServerError(errorMessage);
  }
}
async function auth(provider, options) {
  try {
    ...
    if (error instanceof errors.InvalidClientError || error instanceof errors.UnauthorizedClientError) {
      await provider.invalidateCredentials?.("all");
      return await authInternal(provider, options);
    } else if (error instanceof errors.InvalidGrantError) {
...
async function authInternal(provider, {
  ...
  if (tokens?.refresh_token) {
    try {
      ...
      if (!(error instanceof errors.OAuthError) || error instanceof errors.ServerError) {
      } else {
        throw error;
      }
    }
...
async function selectResourceURL(serverUrl, provider, resourceMetadata) {
  const defaultResource = authUtils.resourceUrlFromServerUrl(serverUrl);
  ...
  if (!authUtils.checkResourceAllowed({ requestedResource: defaultResource, configuredResource: resourceMetadata.resource })) {
...
async function discoverOAuthProtectedResourceMetadata(serverUrl, opts, fetchFn = fetch) {
  ...
  return auth$1.OAuthProtectedResourceMetadataSchema.parse(await response.json());
}
...
async function discoverMetadataWithFallback(serverUrl, wellKnownType, fetchFn, opts) {
  ...
  const protocolVersion = opts?.protocolVersion ?? types.LATEST_PROTOCOL_VERSION;
  let url;
  if (opts?.metadataUrl) {
...
async function discoverOAuthMetadata(issuer, {
  ...
  return auth$1.OAuthMetadataSchema.parse(await response.json());
}
...
async function discoverAuthorizationServerMetadata(authorizationServerUrl, {
  ...
  for (const { url: endpointUrl, type } of urlsToTry) {
    ...
    if (type === "oauth") {
      ...
      const metadata = auth$1.OpenIdProviderDiscoveryMetadataSchema.parse(await response.json());
...
async function exchangeAuthorization(authorizationServerUrl, {
  ...
  return auth$1.OAuthTokensSchema.parse(await response.json());
}
async function refreshAuthorization(authorizationServerUrl, {
  ...
  return auth$1.OAuthTokensSchema.parse({ refresh_token: refreshToken, ...await response.json() });
}
async function registerClient(authorizationServerUrl, {
  ...
  return auth$1.OAuthClientInformationFullSchema.parse(await response.json());
}
