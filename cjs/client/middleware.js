...
const withOAuth = (provider, baseUrl) => (next) => {
  return async (input, init) => {
    ...
    if (response.status === 401) {
      try {
        const resourceMetadataUrl = auth.extractResourceMetadataUrl(response);
        const serverUrl = baseUrl || (typeof input === "string" ? new URL(input).origin : input.origin);
        const result = await auth.auth(provider, {
          serverUrl,
          resourceMetadataUrl,
          fetchFn: next
        });
        if (result === "REDIRECT") {
          throw new auth.UnauthorizedError("Authentication requires user authorization - redirect initiated");
        }
        if (result !== "AUTHORIZED") {
          throw new auth.UnauthorizedError(`Authentication failed with result: ${result}`);
        }
        ...
        throw new auth.UnauthorizedError(`Failed to re-authenticate: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    if (response.status === 401) {
      const url = typeof input === "string" ? input : input.toString();
      throw new auth.UnauthorizedError(`Authentication failed for ${url}`);
    }
    return response;
  };
};
