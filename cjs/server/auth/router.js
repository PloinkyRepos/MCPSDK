...
const revoke = require("./handlers/revoke.js");
const metadata = require("./handlers/metadata.js");
...
function mcpAuthRouter(options) {
  ...
  router.use(
    ...
    authorize.authorizationHandler({ provider: options.provider, ...options.authorizationOptions })
  );
  router.use(new URL(oauthMetadata.token_endpoint).pathname, token.tokenHandler({ provider: options.provider, ...options.tokenOptions }));
  router.use(
  ...
  if (oauthMetadata.registration_endpoint) {
    router.use(
      ...
      register.clientRegistrationHandler({
  ...
  if (oauthMetadata.revocation_endpoint) {
    router.use(
      ...
      revoke.revocationHandler({ provider: options.provider, ...options.revocationOptions })
    );
  }
  return router;
}
function mcpAuthMetadataRouter(options) {
  ...
  router.use(`/.well-known/oauth-protected-resource${rsPath === "/" ? "" : rsPath}`, metadata.metadataHandler(protectedResourceMetadata));
  router.use("/.well-known/oauth-authorization-server", metadata.metadataHandler(options.oauthMetadata));
  return router;
}
