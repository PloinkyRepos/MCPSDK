...
const streamableHttp = require("../../client/streamableHttp.js");
const node_readline = require("node:readline");
const types = require("../../types.js");
const metadataUtils = require("../../shared/metadataUtils.js");
const Ajv = require("ajv");
const readline = node_readline.createInterface({
  input: process.stdin,
...
let notificationsToolLastEventId = void 0;
...
async function connect(url) {
  ...
  try {
    ...
    client.setRequestHandler(types.ElicitRequestSchema, async (request) => {
...
Please provide the following information (attempt ${attempts}/${maxAttempts}):`);
        ...
        for (const [fieldName, fieldSchema] of Object.entries(properties)) {
          ...
          if (field.type === "number" || field.type === "integer") {
            ...
            } else if (field.minimum !== void 0) {
              prompt += ` [min: ${field.minimum}]`;
            } else if (field.maximum !== void 0) {
        ...
        if (Object.keys(content).length !== Object.keys(properties).filter((name) => required.includes(name) || content[name] !== void 0).length) {
    ...
    transport = new streamableHttp.StreamableHTTPClientTransport(new URL(serverUrl), {
      sessionId
    });
    client.setNotificationHandler(types.LoggingMessageNotificationSchema, (notification) => {
      notificationCount++;
      console.log(`
Notification #${notificationCount}: ${notification.params.level} - ${notification.params.data}`);
      process.stdout.write("> ");
    });
    client.setNotificationHandler(types.ResourceListChangedNotificationSchema, async (_) => {
      console.log(`
Resource list changed notification received!`);
      try {
        ...
        const resourcesResult = await client.request(
          ...
          types.ListResourcesResultSchema
        );
...
async function listTools() {
  ...
  try {
    ...
    const toolsResult = await client.request(toolsRequest, types.ListToolsResultSchema);
    console.log("Available tools:");
    if (toolsResult.tools.length === 0) {
      console.log("  No tools available");
    } else {
      for (const tool of toolsResult.tools) {
        console.log(`  - id: ${tool.name}, name: ${metadataUtils.getDisplayName(tool)}, description: ${tool.description}`);
      }
    }
  } catch (error) {
...
async function callTool(name, args) {
  ...
  try {
    ...
    const result = await client.request(request, types.CallToolResultSchema);
...
async function listPrompts() {
  ...
  try {
    ...
    const promptsResult = await client.request(promptsRequest, types.ListPromptsResultSchema);
    console.log("Available prompts:");
    if (promptsResult.prompts.length === 0) {
      console.log("  No prompts available");
    } else {
      for (const prompt of promptsResult.prompts) {
        console.log(`  - id: ${prompt.name}, name: ${metadataUtils.getDisplayName(prompt)}, description: ${prompt.description}`);
      }
    }
  } catch (error) {
...
async function getPrompt(name, args) {
  ...
  try {
    ...
    const promptResult = await client.request(promptRequest, types.GetPromptResultSchema);
    console.log("Prompt template:");
    promptResult.messages.forEach((msg, index2) => {
      console.log(`  [${index2 + 1}] ${msg.role}: ${msg.content.text}`);
    });
  } catch (error) {
...
async function listResources() {
  ...
  try {
    ...
    const resourcesResult = await client.request(resourcesRequest, types.ListResourcesResultSchema);
    console.log("Available resources:");
    if (resourcesResult.resources.length === 0) {
      console.log("  No resources available");
    } else {
      for (const resource of resourcesResult.resources) {
        console.log(`  - id: ${resource.name}, name: ${metadataUtils.getDisplayName(resource)}, description: ${resource.uri}`);
      }
    }
  } catch (error) {
...
async function readResource(uri) {
  ...
  try {
    ...
    const result = await client.request(request, types.ReadResourceResultSchema);
