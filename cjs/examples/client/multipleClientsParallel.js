...
async function createAndRunClient(config) {
  ...
  const transport = new streamableHttp.StreamableHTTPClientTransport(new URL(serverUrl));
  ...
  try {
    ...
    const result = await client.request(toolRequest, types.CallToolResultSchema);
