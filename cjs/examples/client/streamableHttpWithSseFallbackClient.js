...
async function connectWithBackwardsCompatibility(url) {
  ...
  try {
    const streamableTransport = new streamableHttp.StreamableHTTPClientTransport(baseUrl);
    ...
    try {
      const sseTransport = new sse.SSEClientTransport(baseUrl);
      const sseClient = new index.Client({
        name: "backwards-compatible-client",
        version: "1.0.0"
      });
